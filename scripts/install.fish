...
set linked_files .vimrc .vim/vsnip/ruby.json .vim/rc/dein{,_lazy}.toml .config/fish/{config.fish,fish_plugins} .gitconfig .gitignore_global .tmux.conf .bashrc .config/paru/paru.conf .config/waybar/{config,style.css,scripts/ppp.sh} .config/wezterm/wezterm.lua .config/sway/{config,clamshell.sh,ime-vim.sh,recorder.sh}
